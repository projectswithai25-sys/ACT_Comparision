...
if run:
    ...
    # ===== Left: filters and table; Right: details =====
    left, right = st.columns([1.1, 1.9], gap="large")

    with left:
        st.subheader("Filters")
        status_sel = st.multiselect("Status", options=sorted(df["status"].unique()), default=None)
        topic_sel = st.multiselect("Topic", options=sorted(set([x for x in (df["new_topic"].dropna().tolist()+df["old_topic"].dropna().tolist()) if x])), default=None)
        ...
        if section_sel.strip():
            mask = df_view["old_section_ref"].fillna("").str.contains(section_sel, case=False) | \
        ...
        preview_cols = ["status","similarity","old_topic","old_subtopic","old_section_ref","old_subsection_ref","new_topic","new_subtopic","new_section_ref","new_subsection_ref"]
        ...
        for _, r in df_view.iterrows():
            ...
            with st.expander(title, expanded=False):
                if r["status"] == "Added":
                    ...
                    st.markdown(f\"\"\"<div style='max-height:300px;overflow:auto;border:1px solid #eee;padding:12px;border-radius:8px'>
                    { (r['new_text'] or '').replace('<','&lt;').replace('>','&gt;').replace('\\n','<br>') }
                    </div>\"\"\", unsafe_allow_html=True)
                elif r["status"] == "Removed":
                    st.markdown("<div style='font-weight:600'>Removed from New</div>", unsafe_allow_html=True)
                    st.markdown(f\"\"\"<div style='max-height:300px;overflow:auto;border:1px solid #eee;padding:12px;border-radius:8px'>
                    { (r['old_text'] or '').replace('<','&lt;').replace('>','&gt;').replace('\\n','<br>') }
                    </div>\"\"\", unsafe_allow_html=True)
                else:
                    html = inline_diff(r.get("old_text") or "", r.get("new_text") or "")
                    st.markdown(f\"\"\"<div style='max-height:300px;overflow:auto;border:1px solid #eee;padding:12px;border-radius:8px;line-height:1.6'>
                    {html}
                    </div>\"\"\", unsafe_allow_html=True)
    ...
    with c1:
        ...
        excel_bytes.seek(0)
        st.download_button("Excel: Comparison Table", data=excel_bytes, file_name="act_comparison.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with c2:
        csv_bytes = io.StringIO()
        make_csv(matched, csv_bytes)
        st.download_button("CSV: Comparison Table", data=csv_bytes.getvalue(), file_name="act_comparison.csv",
                           mime="text/csv", use_container_width=True)
    with c3:
        docx_bytes = io.BytesIO()
        make_word(matched, docx_bytes, title="Act Comparison Report")
        docx_bytes.seek(0)
        st.download_button("Word: Narrative Report", data=docx_bytes, file_name="act_comparison.docx",
